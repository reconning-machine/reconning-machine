...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Reconning-Machine thinks for you about the meanings.",
        epilog="Example:  reconning-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | reconning-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | reconning-machine         #

    $ reconning-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="reconning-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"reconning-machine: {utterance}.")
        syslog.closelog()
